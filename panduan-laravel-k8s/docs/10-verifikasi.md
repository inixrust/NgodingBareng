...
| # | Yang diperiksa | Perintah | Diharapkan |
|---|---|---|---|
| 2.1 | Tidak ada terminasi TAK NORMAL | `kubectl -n laravel get pods -o jsonpath='{range .items[*].status.containerStatuses[*]}{.lastState.terminated.reason}{"\n"}{end}' \| grep -E 'OOMKilled\|Error'` | kosong |
| 2.2 | Tidak ada Event Warning | `kubectl -n laravel get events --field-selector type=Warning` | kosong |

> **Jangan memakai jumlah RESTARTS mentah sebagai kriteria.** `laravel-queue`
> memang keluar bersih (exit 0) setiap jam karena `--max-time=3600` mendaur
> ulang worker untuk mencegah kebocoran memori — lalu dihidupkan lagi oleh
> `restartPolicy: Always`. Jadi RESTARTS yang tinggi di worker itu **normal**,
> bukan crash. Yang benar-benar gejala buruk adalah terminasi **OOMKilled**
> atau **Error** (exit ≠ 0) — itu tanda memory limit kekecilan, probe terlalu
> ketat, atau aplikasi crash. Karena itu `scripts/verify.sh` memeriksa
> `lastState.terminated.reason`, bukan angka restart (lihat juga
...
echo "=== TERMINASI TAK NORMAL ===" && kubectl -n $NS get pods -o jsonpath='{range .items[*].status.containerStatuses[*]}{.lastState.terminated.reason}{"\n"}{end}' | grep -E 'OOMKilled|Error' || echo "(tidak ada)"
...
- [ ] Tidak ada terminasi `OOMKilled`/`Error` (RESTARTS tinggi pada `laravel-queue` itu normal — daur ulang `--max-time`)
- [ ] Semua PVC `Bound`
